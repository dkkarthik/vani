...
3. In the discovery editor, edit the focused topic or choose **Infer a revised focus from all selected seeds**, then save. Inference uses the local model and includes the first three indexed PDF pages when available. If the local model is unavailable, VANI uses labeled search terms extracted from the seeds. Review or edit that focus after saving. If there is insufficient shared text, VANI saves the collection with discovery paused and asks for a manual focus. Adding a seed does not silently change an existing topic.
