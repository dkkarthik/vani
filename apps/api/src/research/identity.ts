...
export async function mergeWorks(
  ...
  return transaction(async (client) => {
    ...
    await client.query(
      "INSERT INTO collection_membership(collection_id,work_id,status,priority,rationale,ordinal,seen_at,question,queued,inclusion_reason) SELECT collection_id,$2,status,priority,rationale,ordinal,seen_at,question,queued,inclusion_reason FROM collection_membership WHERE canonical_work(work_id)=$1 ON CONFLICT DO NOTHING",
      [sourceId, targetId],
    );
    const relations = (
      await client.query(
