...
## VANI 1.0 implementation

The operational baseline is implemented under `apps/api/src/core` with the shared local inference router under `apps/api/src/models`. See [the release implementation contract](VANI-1.0.md) and [installation, calibration, verification and remaining research gates](../docs/23-vani-1.0.md). These documents distinguish implemented mechanisms from empirical accuracy gates and research extensions; the existence of code does not certify literature-retrieval completeness or target-GPU performance.
