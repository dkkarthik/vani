...
| Collection Q&A           | Retrieve saved assessments and original passages; answer with source links                                                            | Local retrieval and Qwen3.8; direct database answers for simple questions | Local-only for F44 collection conversations, including follow-ups.                              |
...
The user-facing [F44 local collection conversation feature](F44-local-model-collection-conversations.md) is part of this plan. Its task policy is local-only even when other tasks permit cloud escalation under section 5. Implement its conversation context, grounding and recovery behavior alongside the router migration.
